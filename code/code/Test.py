...
class YOLOTester:
    ...
    def preprocess_image(self, image_path: str) -> Tuple[torch.Tensor, np.ndarray, Tuple]:
        ...
        Args:
            image_path: 图像路径
        Returns:
            tensor_image: 预处理后的张量
            original_image: 原始图像
            image_info: 图像信息 (original_height, original_width)
        """
        # 读取图像
    ...
    def postprocess_detections(self, detections: Dict, image_info: Tuple) -> Dict:
        """
        后处理检测结果，将坐标转换回原始图像尺寸
        Args:
            detections: 检测结果
            image_info: 图像信息
        Returns:
            processed_detections: 处理后的检测结果
        """
...
def main():
    ...
    if test_mode == '1':
        ...
        data_config = {
            'type': 'voc',
            'test_imgs_path': './data/VOC2012/JPEGImages',
            'test_annotations_path': './data/VOC2012/Annotations',
            'class_file': './data/voc_classes.txt',
            'input_size': hyperparameters['input_size'],
