...
class BackboneClassificationTrainer:
    ...
    def create_datasets(self, coco_config: Dict):
        ...
        try:
            ...
            # 验证集（如果提供了单独的验证集路径）
            if 'val_imgs_path' in coco_config and 'val_coco_json' in coco_config:
                print("使用独立的验证集...")
                val_dataset = COCO_Segmentation_Classification_Set(
                    imgs_path=coco_config['val_imgs_path'],
                    coco_json=coco_config['val_coco_json'],
                    input_size=self.hyperparameters.get('input_size', 448),
                    min_area=coco_config.get('min_area', 1000),
                    max_objects_per_image=coco_config.get('max_objects_per_image', 10)
                )
    ...
    def train_epoch(self, model, train_loader, criterion, training_optimizer, epoch):
        model.train()
        total_loss = 0.0
        correct = 0
        total = 0
    ...
    def train(self, 
        ...
        training_optimizer = create_adam_optimizer(
            model,
        ...
        if resume_from and os.path.exists(resume_from):
            ...
            training_optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            ...
            self.learning_rates = checkpoint.get('learning_rates', [])  # 恢复学习率记录
        ...
        for epoch in range(start_epoch, epochs):
            # 训练
            train_loss, train_acc = self.train_epoch(model, train_loader, criterion, training_optimizer, epoch)
            
            # 验证
            val_loss, val_acc = self.validate_epoch(model, val_loader, criterion, epoch)
            
            # 学习率调度
            training_optimizer.step(val_loss)  # 传入验证损失给调度器
            current_lr = training_optimizer.get_current_lr()
            ...
            self.learning_rates.append(current_lr)  # 记录当前学习率
            ...
            if val_acc > best_acc:
                ...
                save_checkpoint({
                    ...
                    'optimizer_state_dict': training_optimizer.state_dict(),
                    'best_acc': best_acc,
                    'train_losses': self.train_losses,
                    'train_accuracies': self.train_accuracies,
                    'val_losses': self.val_losses,
                    'val_accuracies': self.val_accuracies,
                    'learning_rates': self.learning_rates,  # 保存学习率记录
...
def main():
    ...
    coco_config = {
        ...
        'val_imgs_path': '../../data/COCO/val2017',  # 验证集图像路径
        'val_coco_json': '../../data/COCO/annotations/instances_val2017.json',  # 验证集标注文件
        'batch_size': 32,  # 增加批次大小，提高训练稳定性
        'learning_rate': 0.0005,  # 降低学习率，减少波动
        'min_area': 1000,
