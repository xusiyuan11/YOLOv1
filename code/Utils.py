...
def decode_yolo_output(predictions: torch.Tensor, 
    ...
    解码 YOLO 输出
    Args:
        predictions: YOLO 输出 (batch_size, grid_size, grid_size, 30)
...
def test_utils():
    ...
    print(f"Default hyperparameters loaded: {len(hyperparams)} items")
    
    # 测试类别名称
    voc_classes = create_class_names('voc')
    print(f"VOC classes: {len(voc_classes)} classes")
