...
class VOC_Detection_Set(Dataset):
    ...
    def __init__(self,
                 ...
                 aug_hsv_gain: Tuple[float, float, float] = (0.015, 0.7, 0.7),  # (h, s, v) 增益范围
                 aug_gaussian_blur_prob: float = 0.1):
        super().__init__()
        ...
        # ---------- 数据增强配置 ----------
        self.aug_hflip_prob = float(max(0.0, min(1.0, aug_hflip_prob)))
        self.aug_hsv_prob = float(max(0.0, min(1.0, aug_hsv_prob)))
        self.aug_hsv_gain = aug_hsv_gain
        self.aug_gaussian_blur_prob = float(max(0.0, min(1.0, aug_gaussian_blur_prob)))
    ...
    def __getitem__(self, idx):
        ...
        # 仅训练集进行数据增强（增强在 pad/resize 之前，保持坐标同步）
        if self.is_train:
            img, coords = self._apply_augmentations(img, coords)
    ...
    def _apply_augmentations(self, image: np.ndarray, coords: List[List]) -> Tuple[np.ndarray, List[List]]:
        """
        轻量级数据增强（几何+颜色），与坐标严格同步。
        - 随机水平翻转
        - 随机 HSV 抖动（H/S/V）
        - 随机高斯模糊（小概率）
        """
        h, w = image.shape[:2]
        aug_img = image
        aug_coords = [c.copy() for c in coords]

        # 1) 随机水平翻转
        if random.random() < self.aug_hflip_prob:
            aug_img = np.ascontiguousarray(np.flip(aug_img, axis=1))
            # 更新坐标：x 方向镜像
            for c in aug_coords:
                x1, y1, x2, y2, cls_id = c
                nx1 = w - x2
                nx2 = w - x1
                c[0], c[2] = nx1, nx2

        # 2) 随机 HSV 抖动（在 RGB→BGR→HSV 路径上操作，兼容 OpenCV）
        if random.random() < self.aug_hsv_prob:
            gain_h, gain_s, gain_v = self.aug_hsv_gain
            # 随机增益因子（H 为加法偏移，S/V 为乘法缩放）
            r_h = (random.random() * 2 - 1) * gain_h * 360.0  # 以度为单位
            r_s = 1.0 + ((random.random() * 2 - 1) * gain_s)
            r_v = 1.0 + ((random.random() * 2 - 1) * gain_v)

            img_bgr = cv2.cvtColor(aug_img, cv2.COLOR_RGB2BGR)
            hsv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV).astype(np.float32)
            # OpenCV HSV: H:[0,180]，此处以比例映射
            hsv[..., 0] = (hsv[..., 0] + (r_h / 2.0)) % 180.0
            hsv[..., 1] = np.clip(hsv[..., 1] * r_s, 0, 255)
            hsv[..., 2] = np.clip(hsv[..., 2] * r_v, 0, 255)
            img_bgr = cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2BGR)
            aug_img = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)

        # 3) 小概率高斯模糊（降低噪声/模拟运动模糊）
        if random.random() < self.aug_gaussian_blur_prob:
            k = random.choice([3, 5])
            aug_img = cv2.GaussianBlur(aug_img, (k, k), sigmaX=0)

        return aug_img, aug_coords
