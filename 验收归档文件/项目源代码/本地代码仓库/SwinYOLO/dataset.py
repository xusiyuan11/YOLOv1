...
def _encode_ground_truth(coords: List[List], input_size: int, grid_size: int, class_num: int, class_smooth_value: float):
    ...
    model_class_num = 20  # 固定为VOC的20个类别
    gt = np.zeros([feature_size, feature_size, num_boxes*5 + model_class_num], dtype=float)
    ...
    for coord in coords:
        ...
        model_class_num = 20
        class_list = np.zeros(model_class_num, dtype=float)
        if 0 <= class_id < model_class_num:
            class_list[class_id] = 1.0

        # SwinYOLO格式：
        # 边界框1: [rel_cx, rel_cy, w, h] 
        # 边界框2: [rel_cx, rel_cy, w, h] (相同，因为只有一个目标)
        # 置信度: [conf1, conf2]
        # 类别: [class_0, class_1, ..., class_19]
        
        ground_truth = np.zeros(num_boxes*5 + model_class_num, dtype=float)
        ...
        gt[idx_row, idx_col] = ground_truth
        mask_pos[idx_row, idx_col, 0] = True
        mask_neg[idx_row, idx_col, 0] = False

    return torch.from_numpy(gt).float(), torch.BoolTensor(mask_pos), torch.BoolTensor(mask_neg)
...
class VOC_Detection_Set(Dataset):
    ...
    def __init__(self,
        ...
        print(f"✅ 加载了 {self.class_num} 个类别: {list(self.class_dict.keys())}")
    ...
    def _encode_ground_truth(coords: List[List],
                             ...
                             smooth: float):
        """
        把坐标编码成 SwinYOLO 格式 (S,S,2*5+C)
        """
        S = grid_size
        num_boxes = 2  # SwinYOLO使用2个边界框
        model_class_num = 20  # 固定为VOC的20个类别
        
        # 格式: [num_boxes*4 + num_boxes + model_class_num] = [8 + 2 + 20] = 30
        gt = torch.zeros((S, S, num_boxes*5 + model_class_num))
