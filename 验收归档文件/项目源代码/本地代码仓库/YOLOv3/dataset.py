...
# 注意：dataset.py保持YOLOv1格式以保证向后兼容性
# 在训练时使用compatibility_fixes进行格式转换
...
def _encode_ground_truth(coords: List[List], input_size: int, grid_size: int, class_num: int, class_smooth_value: float):
    ...
    for coord in coords:
        ...
        # YOLO格式编码：相对于网格的偏移
        tx = (cx * feature_size) - idx_col  # x偏移 [0,1)
        ty = (cy * feature_size) - idx_row  # y偏移 [0,1)
        
        ground_box = [tx, ty, w, h, 1,  # 基本的YOLO格式
...
class VOC_Detection_Set(Dataset):
    ...
    def __init__(self,
                 ...
                 input_size: int = 416,        # YOLOv3标准输入尺寸
                 grid_size: int = 13,          # YOLOv3最小网格尺寸 (416/32=13)
                 is_train: bool = True):
        super().__init__()
    ...
    def _encode_ground_truth(coords: List[List],
                             ...
                             smooth: float):
        """
        把坐标编码成 YOLO 格式 (S,S,5+C)
        输入坐标已经归一化到[0,1]
        """
        S = grid_size
        gt = torch.zeros((S, S, 5 + class_num))
        ...
        for x1, y1, x2, y2, cls_id in coords:
            ...
            # YOLO编码：相对于网格的偏移
            tx = (cx * S) - grid_x  # x偏移 [0,1)
            ty = (cy * S) - grid_y  # y偏移 [0,1)

            gt[grid_y, grid_x, 0:5] = torch.tensor([
                tx,   # tx
                ty,   # ty
                w,    # tw (已归一化)
                h,    # th (已归一化)
