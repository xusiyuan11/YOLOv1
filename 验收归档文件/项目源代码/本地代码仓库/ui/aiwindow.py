...
class AutonomousDrivingUI(QMainWindow, AutonomousDrivingUISetup, CameraVideoHandler):
    ...
    def load_model(self):
        ...
        try:
            ...
            # 基于所选权重所在目录进行本地仓库探测，若无则向上两级查找
            weight_dir = os.path.dirname(weights_path)
            candidate_dirs = []
            try:
                p0 = os.path.abspath(weight_dir)
                p1 = os.path.dirname(p0)
                p2 = os.path.dirname(p1)
                candidate_dirs = [p0, p1, p2]
            except Exception:
                candidate_dirs = [weight_dir]
            ...
            try:
                for cand in candidate_dirs:
                    if not cand or not os.path.isdir(cand):
                        continue
                    if not v5_repo and os.path.isfile(os.path.join(cand, 'hubconf.py')):
                        v5_repo = cand
                    if not v3_repo and os.path.isfile(os.path.join(cand, 'hubconf.py')):
                        v3_repo = cand
                    if not v1_repo and os.path.isfile(os.path.join(cand, 'NetModel.py')):
                        v1_repo = cand
                    if v5_repo or v3_repo or v1_repo:
                        break
            except Exception:
                v5_repo = None
                v3_repo = None
            ...
            if not any([v5_repo, v3_repo, v1_repo]):
                self.console.append(f"ℹ️ 所选目录及其上两级未检测到本地仓库文件（hubconf.py/NetModel.py）：{weight_dir}")
    ...
    def on_image_clicked(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "选择图像文件", r"D:\校企\YOLO复现\code\YOLOv3\checkpoints\detection", "图像文件 (*.png *.jpg *.jpeg *.bmp)"
        )
        if file_path:
    ...
    def on_video_clicked(self):
        """视频检测按钮点击"""
        file_path, _ = QFileDialog.getOpenFileName(
            self, "选择视频文件", r"D:\校企\YOLO复现\code\YOLOv3\checkpoints\detection", "视频文件 (*.mp4 *.avi *.mov *.mkv)"
        )
        if file_path:
            self.stop_all()  # 停止其他模式
    ...
    def _prompt_save_image_dir(self):
        try:
            default_dir = r"D:\校企\YOLO复现\code\pics"
    ...
    def on_select_weight_clicked(self):
        ...
        try:
            ...
            if not start_dir or not os.path.isdir(start_dir):
                base_dir = self._get_base_dir()
                candidate_dirs = [
                    r"D:\校企\YOLO复现\code\YOLOv3\checkpoints\detection",
